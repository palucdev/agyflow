---
name: docs-manager
description: Internal engine for managing project documentation and technical standards in .agyflow/docs/. Handles file operations, INDEX.md generation, and AGENTS.md integration. Invoked by flow-init, standards-update, and standards-discover skills.
user-invocable: false
---

# Documentation Manager (Internal Engine)

Internal skill that manages documentation file operations in `.agyflow/docs/`. Not directly user-invocable — called by `flow-init`, `standards-update`, and `standards-discover` skills.
...
- **AGENTS.md integration is mandatory** — ensures AI reads documentation
...
├── project/                      # Project-level documentation (generated by flow-init, not copied from templates)
...
- **Project documentation** (source of truth): `.agyflow/docs/` in the project root
- **Project configuration**: `AGENTS.md` in the project root

## Capabilities

### 1. Initialize Documentation in Project

Use this when a project doesn't have `.agyflow/docs/` or needs documentation for the first time. This is a **one-time baseline setup** that gives the project a starting point.
...
1. Check if `.agyflow/docs/` exists in the project root
...
3. Create the directory structure based on standards_selection:
   ...
   ```
4. Copy standards to the project's `.agyflow/docs/standards/` directory. **Source selection**: If `standards_source_path` is provided, copy from that external path. Otherwise, copy from this skill's bundled `docs/standards/` directory:
   - **Project documentation**: Do NOT copy project templates — only create the `project/` directory. Project documentation files (vision, roadmap, tech-stack, architecture) are generated by the calling skill (e.g., flow-init) using analyzer data, not copied as placeholder templates.
...
5. Generate INDEX.md with entries for all copied documentation (see "Manage INDEX.md" operation):
   ...
   - Example: If frontend standards are skipped, INDEX.md shows:
     ...
     - _Run `/agyflow:standards-discover --scope=frontend` to auto-discover_
     ```

6. **MANDATORY - Update AGENTS.md:**
   - Check if `AGENTS.md` exists in the project root; if not, ask the user if they want to create it
   - Add the documentation reference section (see "Manage AGENTS.md Integration" operation)
...
- `standards_source_path` (optional, string): Absolute path to an external standards directory (e.g., `/path/to/other-project/.agyflow/docs/standards/`)
...
**Result:** The project now has baseline documentation in `.agyflow/docs/`, a comprehensive INDEX.md, and AGENTS.md integration that ensures AI assistance is documentation-aware. Only selected standard categories are initialized.
...
- `/agyflow:standards-discover` skill to auto-discover standards from codebase
...
1. Scan the `.agyflow/docs/` directory structure
...
5. Write the generated INDEX.md to `.agyflow/docs/INDEX.md`
6. Verify that AGENTS.md references this index (see "Manage AGENTS.md Integration" operation)
...
2. If adding from plugin baseline:
   ...
   - Copy it to the appropriate location in `.agyflow/docs/`
...
2. Check if the documentation exists in `.agyflow/docs/`
...
1. Compare the documentation in this skill's bundled `docs/` directory with the project's `.agyflow/docs/` directory to identify differences
...
6. If the user chooses to copy any documentation:
   - Copy the selected files from this skill's bundled `docs/` directory to the project's `.agyflow/docs/` directory
   - Update INDEX.md to reflect any changes
   - Review AGENTS.md for any necessary updates
...
2. For each bundled document:
   ...
   - Check if it exists in the project at `.agyflow/docs/[category]/[name].md`
...
4. Show whether AGENTS.md has documentation integration
...
### 7. Manage AGENTS.md Integration

Use this to ensure the project's AGENTS.md properly integrates with the documentation system, encouraging AI to read and use the documentation.
...
7. If `.agyflow/docs/` doesn't exist, suggest running the initialization operation first

**Result:** AGENTS.md properly integrates with the documentation system, ensuring AI assistance is documentation-aware and follows team conventions.
...
1. **Check structure:**
   - Verify `.agyflow/docs/` directory exists
   - Verify all expected subdirectories exist (project/, standards/global/, etc.)
2. **Check INDEX.md:**
   - Verify it exists and is readable
   - Check that all files in `.agyflow/docs/` are listed in INDEX.md
   - Check that all files listed in INDEX.md actually exist
   - Report any orphaned files or broken references
3. **Check AGENTS.md integration:**
   - Verify AGENTS.md exists
   - Verify it contains documentation reference section
   - Verify it uses valid file reference format: @.agyflow/docs/INDEX.md (with @ prefix, without backticks)
   - Warn if using incorrect formats like `.agyflow/docs/INDEX.md` or `@.agyflow/docs/INDEX.md` (backticks)
...
7. **Offer to fix issues:**
   ...
   - Fix missing AGENTS.md integration
...
Agent: [Executes Manage INDEX.md - scans .agyflow/docs/, regenerates comprehensive INDEX.md]
```
...
- **AGENTS.md integration is mandatory** — ensures AI reads documentation at task start
- **This skill is an internal engine** — called by flow-init, standards-update, and standards-discover. Not directly user-invocable.
