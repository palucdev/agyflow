...
Update or create standards in `.agyflow/docs/standards/` based on conversation context or a provided description. Automatically detects the best-matching category and file. Supports both baseline categories (global, frontend, backend, testing) and custom user-defined categories.
...
When `--from=PATH` is provided, the skill switches to **sync mode** — importing standards from another project's `.agyflow/docs/standards/` into the current project. This bypasses Phases 1-3 and uses a dedicated flow.
...
Scan `.agyflow/docs/standards/*/` to find all existing categories and standard files. This determines what's available — not limited to baseline categories.
